...
$stmt = $pdo->prepare(
    'SELECT SUM(ts.qty) AS qty, ts.unit_price, SUM(ts.line_total) AS line_total, mi.item_code, mi.item_name
     FROM tst_sales ts JOIN master_item mi ON mi.item_id = ts.item_id
     WHERE ts.sale_id = ?
     GROUP BY ts.item_id, ts.unit_price, mi.item_code, mi.item_name
     ORDER BY mi.item_name, ts.unit_price'
);
$stmt->execute([$saleId]);
$lines = $stmt->fetchAll();
